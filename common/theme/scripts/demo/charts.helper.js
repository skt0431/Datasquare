...
var charts = 
{
	...
	chart_lines_fill_nopoints: 
	{
		...
		init: function()
		{
			...
			$.ajax({
					url : 'rest/search/topcustomers?feature=revenue_m',
					//data : {feature : feat},
					datatype : 'json',
					type : "GET",
					cache : false,
					success : function(sdata){
							for (var i = 1; i < sdata.results.length ; i++) {
								costData.push([i,sdata.results[i].cost]);
								revenueData.push([i,sdata.results[i].revenue]);
								profitData.push([i,sdata.results[i].profit]);
							...
							this.plot = $.plot(
				         		...
				         		}], 
								{
							        ...
							        yaxis: { min: 0 },
							        xaxis: {axisLabel: 'Time Period', ticks:xticks, tickDecimals: 0},
							        colors: [],
							        shadowSize:1,
							        tooltip: true,
									tooltipOpts: {
